...
public class ProdutoController {
	...
	public ResponseEntity<Produto> getById(@PathVariable long id_prod){
		return repositoryP.findById(id_prod).map(resp -> ResponseEntity.ok(resp))
	...
	public ResponseEntity<Produto> post (@RequestBody Produto produto){
		return ResponseEntity.status(HttpStatus.CREATED)
				.body(repositoryP.save(produto));
	}
	
	@PutMapping("/atualizar/{id_prod")
	...
	public void delete(@PathVariable long id_prod) {
		repositoryP.deleteById(id_prod);
	}
